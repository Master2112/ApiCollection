...
class ApiCollection
{
	...
	function ParamArrayFromPostData() //assumes input JSON is identical to paramLayout variable
	{
		$returnVal = [];
		...
		for($i = 1; $i < count($this->paramsLayout); $i++) //1 because a ParamArray does not include an ID
		{
			if(!isset($data->{$this->paramsLayout[$i]}))
				die("ParamArrayFromPostData failed: JSON object did not include required field '" . $this->paramsLayout[$i] . "'.");
			
			array_push($returnVal, $data->{$this->paramsLayout[$i]});
		}
